import scrapy
from ..items import A20225477202JiangxiantingItem
...
class MusicinfoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for li in liList:
            itemML = A20225477202JiangxiantingItem()
        ...
        # 爬取下一页
        if self.offset > 1 and self.offset < 7:
            self.offset += 1
            # 获取下一页的Url地址
    ...
    def SongsListPageParse(self, response):
        ...
        AmountOfPlay = songtbList[0].xpath(
            "./div[@class='more s-fc3']/strong[@id='play-count']/text()").extract_first()
